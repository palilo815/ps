...
int occurrence[max_N + 1];  // occurrence[idx] = Euler_tour에서 vertex idx가 처음 등장한 위치 

int cnt;
int Euler_idx[max_N + 1];   // Euler_idx[u] : u가 Euler_tour에서 새로 부여받은 index
int original_idx[max_N];    // original_idx[u] : Euler_idx가 u인 vertex의 원래 index
